 ...
 *
 * Run:
 *   bunx playwright test tests/cdp/commands/cmd-nav-new-incognito-window.spec.ts \
...
test('cmd_nav_new_incognito_window opens an incognito window', async () => {
    ...
    // Open fixture page so the content script is injected and invoke bridge is ready
    ...
    if (incognitoWindows.length === 0) {
        // Check whether chrome.windows.create would have thrown
            ...
            new Promise<string | null>(resolve => {
                chrome.windows.create({ incognito: true, state: 'minimized' }, (win) => {
                    if (chrome.runtime.lastError) {
                        ...
                        // Created successfully — clean up immediately
                        if (win?.id != null) {
