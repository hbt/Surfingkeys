/**
 * CDP Test: cmd_hints_query_word
 *
 * Comprehensive tests for the hints command 'cq' (Query word with hints).
 * - Command: cmd_hints_query_word
 * - Key: 'cq'
 * - Behavior: Show hints to select and query a word for translation
 * - Focus: Text anchor hint creation, word selection, and query functionality
 *
 ...
 *
 * Note: Uses hints-test.html fixture (self-contained, no external resources)
 *
 * Usage:
 *   Headless:    ./bin/dbg test-run tests/cdp/commands/cmd-hints-query-word.test.ts
 *   Live:        npm run test:cdp:live tests/cdp/commands/cmd-hints-query-word.test.ts
 */
...
describe('cmd_hints_query_word', () => {
    ...
    describe('1.0 Page Setup', () => {
        test('1.1 should have text content on page', async () => {
            ...
            `);
            // hints-test.html has substantial text content
            expect(textContent).toBeGreaterThan(100);
        });
    ...
    describe('2.0 Basic Hint Creation', () => {
        test('2.1 should create hints when pressing cq keys', async () => {
            ...
            // Press 'c' then 'q' to trigger hints
        ...
        test('2.3 should create hints for text anchors matching textAnchorPat', async () => {
            await clickAt(pageWs, 100, 100);
            await sendKey(pageWs, 'c');
            await sendKey(pageWs, 'q');
            await waitForHintCount(5);
            ...
            // Hints are created for text anchors (words)
            expect(hintData.count).toBeGreaterThan(5);
            // Should have reasonable number of text hints
            expect(hintData.count).toBeLessThan(200);
        });
    });
    ...
    describe('4.0 Hint Visibility', () => {
        test('4.1 should have hints with position information', async () => {
            await clickAt(pageWs, 100, 100);
            await sendKey(pageWs, 'c');
            await sendKey(pageWs, 'q');
            await waitForHintCount(5);
            ...
            // Check sample hints have position data
            expect(hintData.sample.length).toBeGreaterThan(0);
            // Note: Text anchor hints may have different visibility behavior
            // Focus on position data rather than offsetParent visibility
            hintData.sample.forEach((hint: any) => {
                // Hints should have position information even if visibility check differs
    ...
    describe('7.0 Edge Cases', () => {
        ...
        test('7.2 should clear hints when pressing cq again', async () => {
            await clickAt(pageWs, 100, 100);
            await sendKey(pageWs, 'c');
            await sendKey(pageWs, 'q');
            await waitForHintCount(5);
            ...
            expect(firstSnapshot.count).toBeGreaterThan(5);

            // Press 'cq' again (should toggle off or recreate)
            await sendKey(pageWs, 'c');
            await sendKey(pageWs, 'q');
    ...
    describe('9.0 Text Content Verification', () => {
        test('9.1 should create hints for words in paragraphs', async () => {
            await clickAt(pageWs, 100, 100);
            await sendKey(pageWs, 'c');
            await sendKey(pageWs, 'q');
            await waitForHintCount(5);
            ...
            // Should have hints for text content
            expect(hintData.found).toBe(true);
            expect(hintData.count).toBeGreaterThan(5);
        });

        test('9.2 should create hints for text matching textAnchorPat pattern', async () => {
            // textAnchorPat: /(^[\n\r\s]*\S{3,}|\b\S{4,})/g
            // Matches: words at start with 3+ chars, or words with 4+ chars
            await clickAt(pageWs, 100, 100);
            await sendKey(pageWs, 'c');
            await sendKey(pageWs, 'q');
            await waitForHintCount(5);
            ...
            // Should have substantial number of text hints
            expect(hintData.count).toBeGreaterThan(10);
            // But not excessive (not every single character)
            expect(hintData.count).toBeLessThan(500);
        });
    });

    describe('10.0 Different Content Types', () => {
        test('10.1 should create hints for text in different sections', async () => {
            await clickAt(pageWs, 100, 100);
            await sendKey(pageWs, 'c');
            await sendKey(pageWs, 'q');
            await waitForHintCount(5);
            ...
            // Should have hints distributed across page sections
            expect(hintData.found).toBe(true);
            expect(hintData.count).toBeGreaterThan(10);

            // Check that hints have varied positions (not all in one spot)
            const positions = hintData.sample.map((h: any) => ({ top: h.position.top, left: h.position.left }));
            const uniqueTops = new Set(positions.map((p: any) => Math.floor(p.top / 50)));

            // Should have hints in multiple vertical positions
            expect(uniqueTops.size).toBeGreaterThan(1);
        });

        test('10.2 should handle text in headings and paragraphs', async () => {
            // Verify page has headings
            const headingCount = await countElements(pageWs, 'h1, h2, h3');
            ...
            // Should create hints for text in various elements
            expect(hintData.count).toBeGreaterThan(10);
        });
    });
});
