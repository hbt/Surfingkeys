/**
 * CDP Test: cmd_hints_image_button
 *
 * Comprehensive tests for the hints command 'f' (Click image or button).
 * - Command: cmd_hints_image_button
 * - Key: 'q'
 * - Behavior: Show hints to click on images and buttons
...
describe('cmd_hints_image_button', () => {
    ...
    const FIXTURE_URL = 'http://127.0.0.1:9873/buttons-images-test.html';
    ...
    beforeAll(async () => {
        ...
        const pageWsUrl = await findContentPage('127.0.0.1:9873/buttons-images-test.html');
    ...
    describe('1.0 Page Setup', () => {
        test('1.1 should have expected number of buttons on page', async () => {
            const buttonCount = await countElements(pageWs, 'button');
            // buttons-images-test.html has ~15+ buttons
            expect(buttonCount).toBeGreaterThan(10);
        });

        test('1.2 should have expected number of images on page', async () => {
            const imgCount = await countElements(pageWs, 'img');
            // buttons-images-test.html has ~8+ images
    ...
    describe('2.0 Basic Hint Creation', () => {
        ...
        test('2.3 should create hints for visible buttons and images only', async () => {
            ...
            // Hints are created for visible buttons and images
            expect(hintData.count).toBeGreaterThan(10);
            expect(hintData.count).toBeLessThan(25); // Reasonable upper bound
        });
    });
    ...
    describe('7.0 Edge Cases', () => {
        ...
        test('7.2 should toggle hints when press q key repeatedly', async () => {
            ...
            // Press 'q' again (may toggle off or recreate)
            await sendKey(pageWs, 'q');
