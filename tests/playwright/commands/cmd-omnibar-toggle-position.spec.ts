...
const SUITE_LABEL = 'cmd_omnibar_toggle_position';
...
test.describe('cmd_omnibar_toggle_position (Playwright)', () => {
    ...
    test.beforeEach(async () => {
        ...
        await callSKApi(page, 'mapcmdkey', '<Ctrl-j>', 'cmd_omnibar_toggle_position');
    });
    ...
    test('cmd_omnibar_toggle_position is invocable with omnibar open', async () => {
        await withPersistedDualCoverage({ suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl }, test.info().title, async () => {
            // Toggle omnibar position between top and bottom
            // Open omnibar first (history mode via 'o' key)
            await page.mouse.click(100, 100);
            await page.keyboard.press('o');
            await waitForOmnibar(page, true);

            const ok = await invokeCommand(page, 'cmd_omnibar_toggle_position');
