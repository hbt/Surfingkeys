...
const SUITE_LABEL = 'cmd_tab_detach_magic_except_active';
...
test.describe('cmd_tab_detach_magic_except_active (Playwright)', () => {
    ...
    test('cmd_tab_detach_magic_except_active detaches all tabs except the active tab into a new window', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/except_active`)}`;
                ...
                await invokeCommand(anchor, 'cmd_tab_detach_magic_except_active');
                ...
                if (DEBUG) console.log(`cmd_tab_detach_magic_except_active: windows ${beforeCount} → ${afterWindows.length}`);
