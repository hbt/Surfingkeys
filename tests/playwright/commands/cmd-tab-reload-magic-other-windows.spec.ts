...
const SUITE_LABEL = 'cmd_tab_reload_magic_other_windows';
...
test.describe('cmd_tab_reload_magic_other_windows (Playwright)', () => {
    ...
    test('cmd_tab_reload_magic_other_windows reloads tabs in other windows, tab count unchanged', async () => {
        const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/other_windows_reload`)}`;
        ...
        await callSKApi(anchor, 'mapcmdkey', 'g-032', 'cmd_tab_reload_magic_other_windows');

        await covBg?.snapshot();
        await covContent?.snapshot();

        await invokeCommand(anchor, 'cmd_tab_reload_magic_other_windows');
        await anchor.waitForTimeout(500);
        ...
        if (DEBUG) console.log(`cmd_tab_reload_magic_other_windows: tab count ${beforeCount} → ${allTabsAfter.length} (unchanged)`);
