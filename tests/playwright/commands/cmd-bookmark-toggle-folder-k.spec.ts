import { test, expect, Page, BrowserContext } from '@playwright/test';
import { launchWithDualCoverage, FIXTURE_BASE, invokeCommand } from '../utils/pw-helpers';
import type { ServiceWorkerCoverage } from '../utils/cdp-coverage';
import { withPersistedDualCoverage } from '../utils/coverage-utils';

async function callSKApi(page: Page, fn: string, ...args: unknown[]) {
    await page.evaluate(([f, a]: [string, unknown[]]) => {
        document.dispatchEvent(new CustomEvent('surfingkeys:api', {
            detail: [f, ...a], bubbles: true, composed: true,
        }));
    }, [fn, args] as [string, unknown[]]);
    await page.waitForTimeout(100);
}

const SUITE_LABEL = 'cmd_bookmark_toggle_folder_k';
const FIXTURE_URL = `${FIXTURE_BASE}/scroll-test.html`;
const KEY = 'bk';
const UNIQUE_ID = 'cmd_bookmark_toggle_folder_k';
const TEST_FOLDER = 'test-bookmark-folder';

let context: BrowserContext;
let page: Page;
let covBg: ServiceWorkerCoverage | undefined;
let initContentCoverageForUrl: ((url: string) => Promise<ServiceWorkerCoverage | undefined>) | undefined;

async function cleanupFolder(ctx: BrowserContext, folderName: string): Promise<void> {
    const sw = ctx.serviceWorkers()[0];
    if (!sw) throw new Error('No service worker found');
    await sw.evaluate((name: string) => {
        return new Promise<void>((resolve) => {
            chrome.bookmarks.search({ title: name }, (results) => {
                const folders = results.filter(r => !r.url && r.title === name);
                if (folders.length === 0) { resolve(); return; }
                let remaining = folders.length;
                for (const f of folders) {
                    chrome.bookmarks.removeTree(f.id, () => {
                        remaining -= 1;
                        if (remaining === 0) resolve();
                    });
                }
            });
        });
    }, folderName);
}

async function getBookmarksInFolder(ctx: BrowserContext, folderName: string): Promise<{ id: string; url?: string }[]> {
    const sw = ctx.serviceWorkers()[0];
    if (!sw) throw new Error('No service worker found');
    return sw.evaluate((name: string) => {
        return new Promise<{ id: string; url?: string }[]>((resolve) => {
            chrome.bookmarks.search({ title: name }, (results) => {
                const folder = results.find(r => !r.url && r.title === name);
                if (!folder) { resolve([]); return; }
                chrome.bookmarks.getChildren(folder.id, (children) => {
                    resolve((children || []).map(c => ({ id: c.id, url: c.url })));
                });
            });
        });
    }, folderName);
}

test.describe('cmd_bookmark_toggle_folder_k (Playwright)', () => {
    test.setTimeout(15_000);

    test.beforeAll(async () => {
        const result = await launchWithDualCoverage(FIXTURE_URL);
        context = result.context;
        covBg = result.covBg;
        initContentCoverageForUrl = result.covForPageUrl;
        page = await context.newPage();
        await page.goto(FIXTURE_URL, { waitUntil: 'load' });
        await page.waitForTimeout(500);
    });

    test.afterAll(async () => {
        await covBg?.close();
        await context?.close();
    });

    test.beforeEach(async () => {
        await cleanupFolder(context, TEST_FOLDER);
        await callSKApi(page, 'unmapAllExcept', []);
        await callSKApi(page, 'mapcmdkey', KEY, UNIQUE_ID);
    });

    test.afterEach(async () => {
        await cleanupFolder(context, TEST_FOLDER);
    });

    test('adds URL to folder when not bookmarked', async () => {
        await withPersistedDualCoverage({ suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl }, test.info().title, async () => {
            const ok = await invokeCommand(page, UNIQUE_ID);
            expect(ok).toBe(true);
            await page.waitForTimeout(300);
            const children = await getBookmarksInFolder(context, TEST_FOLDER);
            expect(children).toHaveLength(1);
            expect(children[0].url).toBe(FIXTURE_URL);
        });
    });

    test('removes URL from folder when already bookmarked', async () => {
        await withPersistedDualCoverage({ suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl }, test.info().title, async () => {
            const sw = context.serviceWorkers()[0];
            await sw.evaluate(({ name, url }: { name: string; url: string }) => {
                return new Promise<void>((resolve) => {
                    chrome.bookmarks.create({ parentId: '1', title: name }, (folder) => {
                        chrome.bookmarks.create({ parentId: folder.id, title: name, url }, () => resolve());
                    });
                });
            }, { name: TEST_FOLDER, url: FIXTURE_URL });

            const ok = await invokeCommand(page, UNIQUE_ID);
            expect(ok).toBe(true);
            await page.waitForTimeout(300);
            const children = await getBookmarksInFolder(context, TEST_FOLDER);
            expect(children).toHaveLength(0);
        });
    });
});
