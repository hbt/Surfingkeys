...
test.describe('sa_baidu_visual (sb — Search selected text with Baidu)', () => {
    ...
    test('sb opens a new tab with baidu search URL', async () => {
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                // Baidu may redirect headless Chrome to a captcha page;
                // the search query is encoded in the backurl in that case.
                // Accept either the direct search URL (wd=) or captcha redirect (wd%3D in backurl).
                const hasDirectWd = url.includes('wd=');
                const hasCaptchaWd = url.includes('wd%3D');
                expect(hasDirectWd || hasCaptchaWd).toBe(true);
