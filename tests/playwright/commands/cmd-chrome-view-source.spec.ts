...
/**
 * cmd_chrome_view_source — opens view-source: of the current page.
 * Uses RUNTIME('viewSource', { tab: { tabbed: true } }) — not tabOpenLink.
 * The new tab URL will be view-source:<current-page-url>.
 */
const SUITE_LABEL = 'cmd_chrome_view_source';
const FIXTURE_URL = `${FIXTURE_BASE}/scroll-test.html`;
const CONTENT_COVERAGE_URL = `${FIXTURE_URL}#cov_cmd_chrome_view_source`;

let context: BrowserContext;
let covBg: ServiceWorkerCoverage | undefined;
let initContentCoverageForUrl: ((url: string) => Promise<ServiceWorkerCoverage | undefined>) | undefined;

test.describe('cmd_chrome_view_source (Playwright)', () => {
    ...
    test('opens view-source: tab for current page', async () => {
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                const invoked = await invokeCommand(page, 'cmd_chrome_view_source');
                expect(invoked, 'invokeCommand must acknowledge cmd_chrome_view_source').toBe(true);
                ...
                const newTab = tabsAfter.find(t => {
                    ...
                    return u.startsWith('view-source:');
                });

                if (newTab) {
                    const tabUrl = newTab.url ?? newTab.pendingUrl ?? '';
                    expect(tabUrl).toMatch(/^view-source:/);
                }
