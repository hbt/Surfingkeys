...
const SUITE_LABEL = 'cmd_tab_detach_magic_children';
...
test.describe('cmd_tab_detach_magic_children (Playwright)', () => {
    ...
    test.beforeEach(async () => {
        ...
        await callSKApi(page, 'mapcmdkey', 'tdk', 'cmd_tab_detach_magic_children');
    });
    ...
    test('cmd_tab_detach_magic_children detaches only child tabs into a new window', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                const parentUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/children`)}`;
                ...
                if (DEBUG) console.log(`cmd_tab_detach_magic_children: windows ${beforeCount} → ${afterWindows.length}`);
