...
const KEY = 'oh';
const UNIQUE_ID = 'sa_github_omnibar';

const SUITE_LABEL = 'sa_github_omnibar';
...
test.describe('sa_github_omnibar (oh — github search omnibar)', () => {
    ...
    test('pressing oh opens github search omnibar', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                // 'oh' is registered via addSearchAlias('h', 'github', ...)
                ...
                if (DEBUG) console.log('Omnibar open state after oh:', opened);
