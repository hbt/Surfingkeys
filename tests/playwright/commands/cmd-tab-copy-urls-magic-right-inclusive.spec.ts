...
const SUITE_LABEL = 'cmd_tab_copy_urls_magic_right_inclusive';
...
test.describe('cmd_tab_copy_urls_magic_right_inclusive (Playwright)', () => {
    ...
    test.beforeEach(async () => {
        ...
        await callSKApi(page, 'mapcmdkey', 'ytE', 'cmd_tab_copy_urls_magic_right_inclusive');
    });
    ...
    test('cmd_tab_copy_urls_magic_right_inclusive copies current and all tabs to the right', async () => {
        await withPersistedDualCoverage(
            ...
            test.info().title,
            async () => {
                // Setup: [left1, anchor, right1, right2]
                // No repeat (repeats==null) → all from anchor rightwards inclusive: [anchorUrl, right1Url, right2Url]
                ...
                const right1Url = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/inclusive_1`)}`;
                const right2Url = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/inclusive_2`)}`;

                await closeAllExcept(await createPage(left1));
                const anchor = await createPage(anchorUrl);
                await createPage(right1Url);
                await createPage(right2Url);
                ...
                // DirectionRightInclusive with no repeat → all right inclusive: anchor + right1 + right2
                await expectClipboardForCommand(anchor, 'cmd_tab_copy_urls_magic_right_inclusive', [anchorUrl, right1Url, right2Url]);
