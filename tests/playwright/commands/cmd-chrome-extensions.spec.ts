...
const SUITE_LABEL = 'cmd_chrome_extensions';
const FIXTURE_URL = `${FIXTURE_BASE}/scroll-test.html`;
const CONTENT_COVERAGE_URL = `${FIXTURE_URL}#cov_cmd_chrome_extensions`;
const EXPECTED_URL_PREFIX = 'chrome://extensions';

let context: BrowserContext;
let covBg: ServiceWorkerCoverage | undefined;
let initContentCoverageForUrl: ((url: string) => Promise<ServiceWorkerCoverage | undefined>) | undefined;

test.describe('cmd_chrome_extensions (Playwright)', () => {
    ...
    test('opens chrome://extensions/ tab', async () => {
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                const invoked = await invokeCommand(page, 'cmd_chrome_extensions');
                expect(invoked, 'invokeCommand must acknowledge cmd_chrome_extensions').toBe(true);
