...
const SUITE_LABEL = 'cmd_tab_detach_magic_all_window';
...
test.describe('cmd_tab_detach_magic_all_window (Playwright)', () => {
    ...
    test.beforeEach(async () => {
        ...
        await callSKApi(page, 'mapcmdkey', 'tdC', 'cmd_tab_detach_magic_all_window');
    });
    ...
    test('cmd_tab_detach_magic_all_window detaches all tabs in the window into a new window', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/all_window`)}`;
                ...
                await invokeCommand(anchor, 'cmd_tab_detach_magic_all_window');
                ...
                if (DEBUG) console.log(`cmd_tab_detach_magic_all_window: windows ${beforeCount} → ${afterWindows.length}`);
