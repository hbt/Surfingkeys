...
const SUITE_LABEL = 'cmd_tab_reload_magic_all_window';
...
test.describe('cmd_tab_reload_magic_all_window (Playwright)', () => {
    ...
    test('cmd_tab_reload_magic_all_window reloads all tabs in window, tab count unchanged', async () => {
        const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/all_window_reload`)}`;
        ...
        await callSKApi(anchor, 'mapcmdkey', 'g-026', 'cmd_tab_reload_magic_all_window');
        ...
        await invokeCommand(anchor, 'cmd_tab_reload_magic_all_window');
        ...
        if (DEBUG) console.log(`cmd_tab_reload_magic_all_window: tab count ${tabsBefore.length} → ${tabsAfter.length} (unchanged)`);
