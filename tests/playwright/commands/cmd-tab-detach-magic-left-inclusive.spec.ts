...
const SUITE_LABEL = 'cmd_tab_detach_magic_left_inclusive';
...
test.describe('cmd_tab_detach_magic_left_inclusive (Playwright)', () => {
    ...
    test('cmd_tab_detach_magic_left_inclusive moves current + all left tabs into a new window', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                // Setup: [left0, left1, anchor, right]
                const left0 = await context.newPage();
                await left0.goto(FIXTURE_URL, { waitUntil: 'load' });
                await closeAllExcept(left0);

                const left0Url = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/left0`)}`;
                await left0.goto(left0Url, { waitUntil: 'load' });
                await left0.waitForTimeout(200);

                const left1 = await context.newPage();
                await left1.goto(FIXTURE_URL, { waitUntil: 'load' });
                await left1.waitForTimeout(200);

                const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/left_inclusive`)}`;
                ...
                await callSKApi(anchor, 'mapcmdkey', 'g-008', 'cmd_tab_detach_magic_left_inclusive');
                ...
                await invokeCommand(anchor, 'cmd_tab_detach_magic_left_inclusive');
