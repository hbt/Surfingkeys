...
const SUITE_LABEL = 'cmd_omnibar_previous_page';
...
test.describe('cmd_omnibar_previous_page (Playwright)', () => {
    ...
    test.beforeEach(async () => {
        ...
        await callSKApi(page, 'mapcmdkey', '<Ctrl-,>', 'cmd_omnibar_previous_page');
    });
    ...
    test('cmd_omnibar_previous_page is invocable with omnibar open', async () => {
        await withPersistedDualCoverage({ suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl }, test.info().title, async () => {
            // Load previous page of omnibar results
            // Open omnibar first (history mode via 'o' key)
            await page.mouse.click(100, 100);
            await page.keyboard.press('o');
            await waitForOmnibar(page, true);

            const ok = await invokeCommand(page, 'cmd_omnibar_previous_page');
