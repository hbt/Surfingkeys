...
const SUITE_LABEL = 'cmd_tab_reload_magic_left';
...
test.describe('cmd_tab_reload_magic_left (Playwright)', () => {
    ...
    test('cmd_tab_reload_magic_left reloads tabs to the left, tab count unchanged', async () => {
        const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/left_reload`)}`;
        ...
        await callSKApi(anchor, 'mapcmdkey', 'g-024', 'cmd_tab_reload_magic_left');

        await covBg?.snapshot();
        await covContent?.snapshot();

        await invokeCommand(anchor, 'cmd_tab_reload_magic_left');
        await anchor.waitForTimeout(500);

        const tabsAfter = await getTabsViaSW(context);
        expect(tabsAfter.length).toBe(tabsBefore.length);
        if (DEBUG) console.log(`cmd_tab_reload_magic_left: tab count ${tabsBefore.length} → ${tabsAfter.length} (unchanged)`);
