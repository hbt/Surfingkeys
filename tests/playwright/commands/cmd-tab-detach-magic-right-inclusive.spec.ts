...
const SUITE_LABEL = 'cmd_tab_detach_magic_right_inclusive';
...
test.describe('cmd_tab_detach_magic_right_inclusive (Playwright)', () => {
    ...
    test('cmd_tab_detach_magic_right_inclusive moves current + all right tabs into a new window', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                // Setup: [left, anchor, right0, right1]
                const left = await context.newPage();
                await left.goto(FIXTURE_URL, { waitUntil: 'load' });
                await closeAllExcept(left);

                const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/right_inclusive`)}`;
                const anchor = await context.newPage();
                await anchor.goto(anchorUrl, { waitUntil: 'load' });
                await anchor.waitForTimeout(200);

                const right0 = await context.newPage();
                await right0.goto(FIXTURE_URL, { waitUntil: 'load' });
                await right0.waitForTimeout(200);
                ...
                await callSKApi(anchor, 'mapcmdkey', 'g-007', 'cmd_tab_detach_magic_right_inclusive');
                ...
                await invokeCommand(anchor, 'cmd_tab_detach_magic_right_inclusive');
                ...
                if (DEBUG) console.log(`cmd_tab_detach_magic_right_inclusive: windows ${beforeCount} → ${afterWindows.length}`);
