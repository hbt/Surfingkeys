...
test.describe('cmd_tab_next (Playwright)', () => {
    ...
    test('pressing bound key moves active tab from index 0 to index 1', async () => {
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                await callSKApi(tab0, 'mapcmdkey', 'gt', 'cmd_tab_next');
                ...
                await tab0.keyboard.press('g');
                await tab0.waitForTimeout(50);
                await tab0.keyboard.press('t');
                await tab0.waitForTimeout(500);

                const after = await getActiveTabViaSW(context);
                expect(after.index).toBe(1);
                if (DEBUG) console.log(`cmd_tab_next_key: index ${before.index} → ${after.index}`);
            },
        );
    });

    test('pressing bound key wraps from last tab to index 0', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                ...
                await callSKApi(tab1, 'mapcmdkey', 'gt', 'cmd_tab_next');
                ...
                await tab1.keyboard.press('g');
                await tab1.waitForTimeout(50);
                await tab1.keyboard.press('t');
                await tab1.waitForTimeout(500);

                const after = await getActiveTabViaSW(context);
                expect(after.index).toBe(0);
                if (DEBUG) console.log(`cmd_tab_next_key wrap: index ${before.index} → ${after.index}`);
            },
        );
    });
});
