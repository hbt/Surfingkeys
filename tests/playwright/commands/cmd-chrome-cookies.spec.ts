...
const SUITE_LABEL = 'cmd_chrome_cookies';
const FIXTURE_URL = `${FIXTURE_BASE}/scroll-test.html`;
const CONTENT_COVERAGE_URL = `${FIXTURE_URL}#cov_cmd_chrome_cookies`;
const EXPECTED_URL_PREFIX = 'chrome://settings';

let context: BrowserContext;
let covBg: ServiceWorkerCoverage | undefined;
let initContentCoverageForUrl: ((url: string) => Promise<ServiceWorkerCoverage | undefined>) | undefined;

test.describe('cmd_chrome_cookies (Playwright)', () => {
    ...
    test('opens chrome://settings/cookies tab', async () => {
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                const invoked = await invokeCommand(page, 'cmd_chrome_cookies');
                expect(invoked, 'invokeCommand must acknowledge cmd_chrome_cookies').toBe(true);
