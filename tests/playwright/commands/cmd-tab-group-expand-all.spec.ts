...
const SUITE_LABEL = 'cmd_tab_group_expand_all';
...
test.describe('cmd_tab_group_expand_all (Playwright)', () => {
    ...
    test('two collapsed groups → invoke → both expanded', async () => {
        await withPersistedDualCoverage({ suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl }, test.info().title, async () => {
            ...
            // Collapse both groups explicitly
            await setGroupCollapsed(context, groupId1, true);
            await setGroupCollapsed(context, groupId2, true);
            await page.waitForTimeout(200);
            ...
            expect(g1Before?.collapsed).toBe(true);
            expect(g2Before?.collapsed).toBe(true);

            // Invoke expand all
            await invokeCommand(page, 'cmd_tab_group_expand_all');
            ...
            expect(g1After?.collapsed).toBe(false);
            expect(g2After?.collapsed).toBe(false);

            // Cleanup: ungroup and close extra tab
