...
test.describe('cmd_tab_close (Playwright)', () => {
    ...
    test('cmd_tab_close closes the active tab', async () => {
        const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/close`)}`;
        ...
        await callSKApi(anchor, 'mapcmdkey', 'x', 'cmd_tab_close');
        ...
        if (DEBUG) console.log(`tab count: ${tabsBefore.length} → ${tabsAfter.length}`);
