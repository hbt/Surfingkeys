...
const SUITE_LABEL = 'cmd_tab_detach_magic_other_windows_key';
...
test.describe('cmd_tab_detach_magic_other_windows_key (Playwright)', () => {
    ...
    test('cmd_tab_detach_magic_other_windows detaches tabs in other windows via tdW', async () => {
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                const otherWinId = await openWindowViaSW(context, FIXTURE_URL);
                ...
                }, { windowId: otherWinId, url: FIXTURE_URL });
                await anchor.waitForTimeout(500);
                ...
                await callSKApi(anchor, 'mapcmdkey', 'tdW', 'cmd_tab_detach_magic_other_windows');

                const beforeTabs = await getAllWindowsViaSW(context);
                const beforeCount = beforeTabs.length;
                const currentWindowId = beforeTabs.find((w: any) => w.id !== otherWinId)?.id ?? beforeTabs[0].id;
                ...
                const detachedWindow = afterWindows.find((w: any) => w.id !== currentWindowId && w.id !== otherWinId);

                expect(currentWindow).toBeDefined();
                expect(detachedWindow).toBeDefined();
                expect(currentWindow!.tabs.length).toBe(1);
                expect(detachedWindow!.tabs.length).toBe(2);

                if (DEBUG) console.log(`cmd_tab_detach_magic_other_windows_key: windows ${beforeCount} → ${afterWindows.length}`);
                ...
                const swCleanup = context.serviceWorkers()[0];
                await swCleanup.evaluate((id: number) => {
                    return new Promise<void>((resolve) => {
