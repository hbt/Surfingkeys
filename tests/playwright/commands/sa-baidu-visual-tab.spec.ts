...
const KEY = 'sB';
const UNIQUE_ID = 'sa_baidu_visual_tab';

const SUITE_LABEL = 'sa_baidu_visual_tab';
...
test.describe('sa_baidu_visual_tab (sB — Search selected with Baidu in omnibar)', () => {
    ...
    test('sB opens baidu search omnibar (interactive mode)', async () => {
        ...
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                await page.waitForTimeout(100);
                // sB = interactive mode → opens omnibar pre-filled with selected text
                await page.keyboard.press('s');
                await page.waitForTimeout(50);
                await page.keyboard.press('B');  // capital B
                const opened = await waitForOmnibarState(page, true);
                if (DEBUG) console.log('Omnibar open state after sB:', opened);
