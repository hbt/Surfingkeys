...
const SUITE_LABEL = 'cmd_tab_detach_magic_children_recursive_key';
...
test.describe('cmd_tab_detach_magic_children_recursive_key (Playwright)', () => {
    ...
    test('cmd_tab_detach_magic_children_recursive detaches child + grandchild tabs via tdK', async () => {
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                await callSKApi(parent, 'mapcmdkey', 'tdK', 'cmd_tab_detach_magic_children_recursive');
                ...
                expect(remainingTabs.has(childId)).toBe(true);
                expect(remainingTabs.has(grandchildId)).toBe(true);

                if (DEBUG) console.log(`cmd_tab_detach_magic_children_recursive_key: windows ${beforeCount} → ${afterWindows.length}`);
