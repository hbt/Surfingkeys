...
const SUITE_LABEL = 'cmd_tab_copy_urls_magic_except_active';
...
test.describe('cmd_tab_copy_urls_magic_except_active (Playwright)', () => {
    ...
    test('cmd_tab_copy_urls_magic_except_active copies all tabs except the active tab', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                // Setup: [extra1, anchor, extra2] where anchor is active
                const extra1Url = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/extra1`)}`;
                const extra1 = await createPage(extra1Url);
                await closeAllExcept(extra1);

                const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/anchor`)}`;
                const anchor = await createPage(anchorUrl);

                const extra2Url = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/extra2`)}`;
                await createPage(extra2Url);

                await anchor.bringToFront();
                await anchor.waitForTimeout(200);
                await callSKApi(anchor, 'unmapAllExcept', []);
                await callSKApi(anchor, 'mapcmdkey', 'gycc', 'cmd_tab_copy_urls_magic_except_active');

                const covContent = await initContentCoverageForUrl?.(anchorUrl);
                const expectedUrls = [extra1Url, extra2Url];

                await covBg?.snapshot();
                await covContent?.snapshot();
                await expectClipboardForCommand(anchor, 'cmd_tab_copy_urls_magic_except_active', expectedUrls);
