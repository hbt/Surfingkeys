...
const SUITE_LABEL = 'cmd_tab_reload_magic_other_windows_no_pinned';
...
test.describe('cmd_tab_reload_magic_other_windows_no_pinned (Playwright)', () => {
    ...
    test('cmd_tab_reload_magic_other_windows_no_pinned reloads other windows without pinned tabs, tab count unchanged', async () => {
        const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/other_windows_no_pinned_reload`)}`;
        const anchor = await context.newPage();
        await anchor.goto(anchorUrl, { waitUntil: 'load' });
        await closeAllExcept(anchor);

        // Open second window (no pinned tabs — eligible for reload)
        ...
        await callSKApi(anchor, 'mapcmdkey', 'g-033', 'cmd_tab_reload_magic_other_windows_no_pinned');

        await covBg?.snapshot();
        await covContent?.snapshot();

        await invokeCommand(anchor, 'cmd_tab_reload_magic_other_windows_no_pinned');
        await anchor.waitForTimeout(500);
        ...
        if (DEBUG) console.log(`cmd_tab_reload_magic_other_windows_no_pinned: tab count ${beforeCount} → ${allTabsAfter.length} (unchanged)`);
