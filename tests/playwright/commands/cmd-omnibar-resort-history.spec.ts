...
const SUITE_LABEL = 'cmd_omnibar_resort_history';
...
test.describe('cmd_omnibar_resort_history (Playwright)', () => {
    ...
    test.beforeEach(async () => {
        ...
        await callSKApi(page, 'mapcmdkey', '<Ctrl-r>', 'cmd_omnibar_resort_history');
    });
    ...
    test('cmd_omnibar_resort_history is invocable with omnibar open', async () => {
        await withPersistedDualCoverage({ suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl }, test.info().title, async () => {
            // Re-sort history results in omnibar
            // Open omnibar first (history mode via 'o' key)
            await page.mouse.click(100, 100);
            await page.keyboard.press('o');
            await waitForOmnibar(page, true);

            const ok = await invokeCommand(page, 'cmd_omnibar_resort_history');
