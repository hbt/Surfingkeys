...
const SUITE_LABEL = 'cmd_tab_detach_magic_right_key';
...
test.describe('cmd_tab_detach_magic_right_key (Playwright)', () => {
    ...
    test('cmd_tab_detach_magic_right detaches tabs to the right into a new window via tde', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                const anchorUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/via_tde`)}`;
                ...
                // Install the user-config mapping in the test harness, then exercise it via keys.
                await callSKApi(anchor, 'unmapAllExcept', []);
                await callSKApi(anchor, 'mapcmdkey', 'tde', 'cmd_tab_detach_magic_right');
                ...
                if (DEBUG) console.log(`cmd_tab_detach_magic_right_key: windows ${beforeCount} → ${afterWindows.length}`);
