...
const KEY = 'sD';
const UNIQUE_ID = 'sa_duckduckgo_visual_tab';

const SUITE_LABEL = 'sa_duckduckgo_visual_tab';
...
test.describe('sa_duckduckgo_visual_tab (sD — Search selected with DuckDuckGo in omnibar)', () => {
    ...
    test('sD opens duckduckgo search omnibar (interactive mode)', async () => {
        await withPersistedDualCoverage(
            ...
            async () => {
                ...
                await page.waitForTimeout(100);
                // sD = interactive mode → opens omnibar pre-filled with selected text
                await page.keyboard.press('s');
                await page.waitForTimeout(50);
                await page.keyboard.press('D');  // capital D
                const opened = await waitForOmnibarState(page, true);
                if (DEBUG) console.log('Omnibar open state after sD:', opened);
