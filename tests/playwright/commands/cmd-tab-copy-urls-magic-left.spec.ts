...
const SUITE_LABEL = 'cmd_tab_copy_urls_magic_left';
...
test.describe('cmd_tab_copy_urls_magic_left (Playwright)', () => {
    ...
    test('cmd_tab_copy_urls_magic_left preserves tabHandleMagic order with repeats', async () => {
        await withPersistedDualCoverage(
            ...
            test.info().title,
            async () => {
                // Setup: [left2, left1, anchor, right1, right2]
                // repeat=2 → closest 2 to the left (reversed): [left1, left2]
                ...
                await callSKApi(anchor, 'mapcmdkey', 'g-016', 'cmd_tab_copy_urls_magic_left');
