...
const KEY = 'ow';
const UNIQUE_ID = 'sa_bing_omnibar';

const SUITE_LABEL = 'sa_bing_omnibar';
...
test.describe('sa_bing_omnibar (ow — bing search omnibar)', () => {
    ...
    test('pressing ow opens bing search omnibar', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                // 'ow' is registered via addSearchAlias('w', 'bing', ...)
                ...
                if (DEBUG) console.log('Omnibar open state after ow:', opened);
