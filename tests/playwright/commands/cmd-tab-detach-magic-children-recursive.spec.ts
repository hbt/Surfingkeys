...
const SUITE_LABEL = 'cmd_tab_detach_magic_children_recursive';
...
test.describe('cmd_tab_detach_magic_children_recursive (Playwright)', () => {
    ...
    test.beforeEach(async () => {
        ...
        await callSKApi(page, 'mapcmdkey', 'tdK', 'cmd_tab_detach_magic_children_recursive');
    });
    ...
    test('cmd_tab_detach_magic_children_recursive detaches child + grandchild tabs into a new window', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: CONTENT_COVERAGE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                const parentUrl = `${FIXTURE_URL}#${coverageSlug(`${SUITE_LABEL}/children_recursive`)}`;
                ...
                await parent.waitForTimeout(300);
                await openChildTabViaSW(context, childId, FIXTURE_URL);
                ...
                await invokeCommand(parent, 'cmd_tab_detach_magic_children_recursive');
                ...
                if (DEBUG) console.log(`cmd_tab_detach_magic_children_recursive: windows ${beforeCount} → ${afterWindows.length}`);
