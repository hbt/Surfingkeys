...
async function invokeVisualWordEnd(p: Page) {
    const ok = await invokeCommand(p, 'cmd_visual_word_end');
    expect(ok).toBe(true);
}

test.describe('cmd_visual_word_end (Playwright)', () => {
    ...
    test.afterAll(async () => {
        if (cov) printCoverageDelta(await cov.delta(), 'cmd_visual_word_end');
        await cov?.close();
        await context?.close();
    });
    ...
    test('pressing e multiple times does not error', async () => {
        ...
        const sel = await getSelectionInfo(page);
        expect(sel.hasNode).toBe(true);
        if (DEBUG) console.log(`After 5x e: focusOffset=${sel.focusOffset}`);
    });
    ...
    test('e and w do not error in sequence', async () => {
        ...
        const sel = await getSelectionInfo(page);
        expect(sel.hasNode).toBe(true);
        if (DEBUG) console.log(`e then w: type=${sel.type}`);
    });
    ...
    test('visual mode remains accessible after pressing e', async () => {
        ...
        if (DEBUG) console.log(`After e then j: ${before} → ${after}`);
    });
});
