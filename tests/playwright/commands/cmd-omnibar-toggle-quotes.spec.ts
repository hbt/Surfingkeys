...
test.describe('cmd_omnibar_toggle_quotes (Playwright)', () => {
    ...
    test('cmd_omnibar_toggle_quotes is invocable with omnibar open', async () => {
        await withPersistedDualCoverage({ suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl }, test.info().title, async () => {
            // Toggle quotes in omnibar input
            // Open omnibar first (history mode via 'o' key)
            await page.mouse.click(100, 100);
            await page.keyboard.press('o');
            await waitForOmnibar(page, true);

            const ok = await invokeCommand(page, 'cmd_omnibar_toggle_quotes');
