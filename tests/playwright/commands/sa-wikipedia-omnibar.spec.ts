...
const KEY = 'oe';
const UNIQUE_ID = 'sa_wikipedia_omnibar';

const SUITE_LABEL = 'sa_wikipedia_omnibar';
...
test.describe('sa_wikipedia_omnibar (oe — wikipedia search omnibar)', () => {
    ...
    test('pressing oe opens wikipedia search omnibar', async () => {
        await withPersistedDualCoverage(
            { suiteLabel: SUITE_LABEL, coverageUrl: FIXTURE_URL, covBg, initContentCoverageForUrl },
            test.info().title,
            async () => {
                // 'oe' is registered via addSearchAlias('e', 'wikipedia', ...)
                ...
                if (DEBUG) console.log('Omnibar open state after oe:', opened);
