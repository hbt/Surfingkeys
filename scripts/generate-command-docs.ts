...
function extractMappings(content: string, type = 'mapkey') {
    const mappings = [];
...
function extractOmnibarMappings(content: string) {
    const mappings = [];
...
function formatMarkdownTable(rows: string[][]) {
    ...
    const numCols = Math.max(...rows.map((r: string[]) => r.length));
    ...
    // Format each row
    const formattedLines = rows.map((row: string[], rowIdx: number) => {
...
function generateMarkdown(mappings: Record<string, unknown>[], commands: Record<string, unknown>[]) {
    ...
    modes.forEach(mode => {
        const modeMappings = mappings.filter((m: Record<string, unknown>) => m.mode === mode);
        if (modeMappings.length === 0) return;

        md += `### ${mode} Mode\n\n`;

        // Group by feature group
        const byFeatureGroup: Record<string, Record<string, unknown>[]> = {};
        modeMappings.forEach((m: Record<string, unknown>) => {
            const fg = String(m.featureGroup);
            if (!byFeatureGroup[fg]) {
                byFeatureGroup[fg] = [];
            }
            byFeatureGroup[fg].push(m);
        });
        ...
        sortedGroups.forEach(groupId => {
            const groupName = (FEATURE_GROUPS as Record<string, string>)[groupId] || 'Misc';
            ...
            items.forEach((item: Record<string, unknown>) => {
                const key = '`' + String(item.key).replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\|/g, '\\|') + '`';
    ...
    // Omnibar mappings
    const omnibarMappings = mappings.filter((m: Record<string, unknown>) => m.mode === 'Omnibar');
    if (omnibarMappings.length > 0) {
        ...
        omnibarMappings.forEach((item: Record<string, unknown>) => {
            const key = '`' + String(item.key).replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\|/g, '\\|') + '`';
            const desc = String(item.description).replace(/\|/g, '\\|');
            tableRows.push([key, desc]);
        });
    ...
    const commandsByGroup: Record<string, Record<string, unknown>[]> = {};
    commands.forEach((cmd: Record<string, unknown>) => {
        const fg = String(cmd.featureGroup);
        if (!commandsByGroup[fg]) {
            commandsByGroup[fg] = [];
        }
        commandsByGroup[fg].push(cmd);
    });
    ...
    sortedCommandGroups.forEach(groupId => {
        ...
        items.forEach((cmd: Record<string, unknown>) => {
            const name = '`' + String(cmd.name).replace(/\|/g, '\\|') + '`';
            const desc = String(cmd.description).replace(/\|/g, '\\|');
