...
function start(browser: Record<string, unknown>) {
    ...
    self.closeTabMagic = function(message: Msg, sender: chrome.runtime.MessageSender, _sendResponse: (response: unknown) => void) {
        ...
        // AllIncognitoTabs: signal incognito CSes via SSE relay to close themselves.
        if (magic === 'AllIncognitoTabs') {
            fetch(`http://localhost:${__CONFIG_SERVER_PORT__}/close-incognito`, { method: 'POST' })
                .then(r => r.json())
                .then(data => debugLog('closeTabMagic', `AllIncognitoTabs: sent to ${data.sent} subscriber(s)`, data))
                .catch(err => debugLog('closeTabMagic', `AllIncognitoTabs: server unreachable`, { error: String(err) }));
            return;
        }
