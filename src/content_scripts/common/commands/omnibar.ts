...
export default function registerOmnibar(
    ...
    mapkey('Q', {
        ...
        fr.openOmniquery({query: getWordUnderCursor() ?? '', style: "opacity: 0.8;"});
    });
    ...
    mapkey('go', {
        ...
        fr.openOmnibar({type: "URLs", tabbed: false});
    });
    mapkey('ox', {
        ...
        fr.openOmnibar({type: "RecentlyClosed"});
    });
    mapkey('b', {
    ...
    mapkey('ab', {
        ...
        fr.openOmnibar({type: "AddBookmark", extra: page});
    });
    mapkey('oh', {
    ...
    addSearchAlias('e', 'wikipedia', 'https://en.wikipedia.org/wiki/', 's', 'https://en.wikipedia.org/w/api.php?action=opensearch&format=json&formatversion=2&namespace=0&limit=40&search=', function(response) {
        return (JSON.parse(response) as [unknown, string[]])[1];
    });
    ...
    addSearchAlias('h', 'github', 'https://github.com/search?q=', 's', 'https://api.github.com/search/repositories?order=desc&q=', function(response) {
        var res = (JSON.parse(response) as { items?: { description: string; html_url: string }[] })['items'];
    ...
    'https://clients1.google.com/complete/search?client=youtube&ds=yt&callback=cb&q=', function(response) {
        var res = JSON.parse(response.substr(9, response.length-10)) as [unknown, [string][]];
